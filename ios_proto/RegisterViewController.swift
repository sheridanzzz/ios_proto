        ...
        if let email = email_textField.text, let password = retype_pass_textField.text {
            Auth.auth().createUser(withEmail: email, password: password) { authResult, error in
                if let e = error {
                    ...
                    db.collection("users").addDocument(data: ["firstName":firstName, "lastName":lastName, "gender":self.gender_text, "state":state, "postcode":postcode, "uuid":authResult?.user.uid, "dateOfBirth":self.dateText, "registerationDate":dateFormatter.string(from: date)]) { (error) in
